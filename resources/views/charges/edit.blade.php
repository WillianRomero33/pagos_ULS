...
<div class="flex">
  ...
  <main class="flex-1 p-6">
    <h1 class="text-2xl font-bold mb-4">{{ isset($fee) ? 'Editar' : 'Nueva' }} Cargo</h1>
    <form method="POST" action="{{ isset($fee) ? route('fees.update', $fee) : route('fees.store') }}">
      ...
      <div class="mb-4">
        ...
        <input name="amount" type="number" step="0.01" class="w-full border rounded px-4 py-2" value="{{ old('amount', $fee->amount ?? '') }}" required>
      </div>

      <div class="mb-4">
        <label class="block font-semibold">Descripción</label>
        <input name="description" type="text" class="w-full border rounded px-4 py-2" value="{{ old('description', $fee->description ?? '') }}" required>
      </div>
