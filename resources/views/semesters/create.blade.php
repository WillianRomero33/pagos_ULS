  ...
  <main class="flex-1 p-6">
    ...
    <form method="POST" action="{{ isset($semester) ? route('semesters.update', $semester) : route('semesters.store') }}">
      ...
      <div class="mb-4">
        ...
        <input name="semester_number" type="number" class="w-full border rounded px-4 py-2" required>
      </div>

      <div class="mb-4">
        <label class="block font-semibold">Duración (meses)</label>
        <input name="duration_months" type="number" class="w-full border rounded px-4 py-2" required>
      </div>
